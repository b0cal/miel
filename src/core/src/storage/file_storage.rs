...
impl FileStorage {
    pub fn new<P: AsRef<Path>>(base_path: P) -> Result<Self, StorageError> {
        ...
        fs::create_dir_all(&sessions_dir).map_err(|_| StorageError::WriteFailed)?;
        fs::create_dir_all(&interactions_dir).map_err(|_| StorageError::WriteFailed)?;
        fs::create_dir_all(&artifacts_path).map_err(|_| StorageError::WriteFailed)?;
    ...
    fn write_session_file(&self, session: &Session) -> Result<(), StorageError> {
        ...
        let mut f = File::create(&path).map_err(|_| StorageError::WriteFailed)?;
        // Simple KV text format
        writeln!(f, "id: {}", session.id).map_err(|_| StorageError::WriteFailed)?;
        writeln!(f, "service_name: {}", session.service_name).map_err(|_| StorageError::WriteFailed)?;
        writeln!(f, "client_addr: {}", session.client_addr).map_err(|_| StorageError::WriteFailed)?;
        writeln!(f, "start_time: {}", session.start_time.to_rfc3339()).map_err(|_| StorageError::WriteFailed)?;
        if let Some(end) = session.end_time { writeln!(f, "end_time: {}", end.to_rfc3339()).map_err(|_| StorageError::WriteFailed)?; } else { writeln!(f, "end_time: none").map_err(|_| StorageError::WriteFailed)?; }
        if let Some(ref cid) = session.container_id { writeln!(f, "container_id: {}", cid).map_err(|_| StorageError::WriteFailed)?; } else { writeln!(f, "container_id: none").map_err(|_| StorageError::WriteFailed)?; }
        writeln!(f, "bytes_transferred: {}", session.bytes_transferred).map_err(|_| StorageError::WriteFailed)?;
        ...
        writeln!(f, "status: {}", status_str).map_err(|_| StorageError::WriteFailed)?;

        // update index
        if let Ok(mut idx) = self.session_index.lock() { idx.insert(session.id, path); }
        Ok(())
    }

    fn parse_session_file(&self, path: &Path) -> Result<Session, StorageError> {
        let mut content = String::new();
        File::open(path).and_then(|mut f| f.read_to_string(&mut content)).map_err(|_| StorageError::ReadFailed)?;
        let mut map: HashMap<String, String> = HashMap::new();
        for line in content.lines() {
            if let Some((k, v)) = line.split_once(": ") { map.insert(k.trim().to_string(), v.trim().to_string()); }
        }
        let id = map.get("id").ok_or(StorageError::ReadFailed)?.parse::<Uuid>().map_err(|_| StorageError::ReadFailed)?;
        let service_name = map.remove("service_name").ok_or(StorageError::ReadFailed)?;
        let client_addr_str = map.remove("client_addr").ok_or(StorageError::ReadFailed)?;
        let client_addr = client_addr_str.parse().map_err(|_| StorageError::ReadFailed)?;
        let start_time = map.remove("start_time").ok_or(StorageError::ReadFailed).and_then(|s| DateTime::parse_from_rfc3339(&s).map(|dt| dt.with_timezone(&Utc)).map_err(|_| StorageError::ReadFailed))?;
        let end_time = map.remove("end_time").and_then(|s| if s == "none" { None } else { Some(s) }).map(|s| DateTime::parse_from_rfc3339(&s).map(|dt| dt.with_timezone(&Utc)).map_err(|_| StorageError::ReadFailed)).transpose()?;
        let container_id = map.remove("container_id").and_then(|s| if s == "none" { None } else { Some(s) });
        let bytes_transferred = map.remove("bytes_transferred").ok_or(StorageError::ReadFailed)?.parse::<u64>().map_err(|_| StorageError::ReadFailed)?;
        let status = match map.remove("status").ok_or(StorageError::ReadFailed)?.as_str() {
...
impl Storage for FileStorage {
    ...
    fn get_sessions(&self, filter: Option<SessionFilter>) -> Result<Vec<Session>, StorageError> {
        ...
        for entry in fs::read_dir(self.sessions_dir()).map_err(|_| StorageError::ReadFailed)? {
            let entry = entry.map_err(|_| StorageError::ReadFailed)?;
            let path = entry.path();
    ...
    fn save_interaction(&self, session_id: Uuid, data: &[u8]) -> Result<(), StorageError> {
        ...
        let mut f = OpenOptions::new().create(true).append(true).open(&path).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(data).map_err(|_| StorageError::WriteFailed)
    }

    fn get_session_data(&self, session_id: Uuid) -> Result<Vec<u8>, StorageError> {
        let path = self.interactions_dir().join(format!("{}.bin", session_id));
        let mut buf = Vec::new();
        File::open(&path).and_then(|mut f| f.read_to_end(&mut buf)).map_err(|_| StorageError::ReadFailed)?;
        Ok(buf)
    }
    ...
    fn save_capture_artifacts(&self, artifacts: &CaptureArtifacts) -> Result<(), StorageError> {
        ...
        // tcp dirs
        let mut f = File::create(dir.join("tcp_client_to_container.bin")).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(&artifacts.tcp_client_to_container).map_err(|_| StorageError::WriteFailed)?;
        let mut f = File::create(dir.join("tcp_container_to_client.bin")).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(&artifacts.tcp_container_to_client).map_err(|_| StorageError::WriteFailed)?;
        // stdio
        let mut f = File::create(dir.join("stdio_stdin.bin")).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(&artifacts.stdio_stdin).map_err(|_| StorageError::WriteFailed)?;
        let mut f = File::create(dir.join("stdio_stdout.bin")).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(&artifacts.stdio_stdout).map_err(|_| StorageError::WriteFailed)?;
        let mut f = File::create(dir.join("stdio_stderr.bin")).map_err(|_| StorageError::WriteFailed)?;
        f.write_all(&artifacts.stdio_stderr).map_err(|_| StorageError::WriteFailed)?;
        // timestamps CSV-like
        let mut f = File::create(dir.join("tcp_timestamps.csv")).map_err(|_| StorageError::WriteFailed)?;
        for (ts, dirn, sz) in &artifacts.tcp_timestamps {
            ...
            writeln!(f, "{}, {}, {}", ts.to_rfc3339(), d, sz).map_err(|_| StorageError::WriteFailed)?;
        }
        let mut f = File::create(dir.join("stdio_timestamps.csv")).map_err(|_| StorageError::WriteFailed)?;
        for (ts, stream, sz) in &artifacts.stdio_timestamps {
            ...
            writeln!(f, "{}, {}, {}", ts.to_rfc3339(), s, sz).map_err(|_| StorageError::WriteFailed)?;
        }
        // meta
        let mut f = File::create(dir.join("meta.txt")).map_err(|_| StorageError::WriteFailed)?;
        writeln!(f, "session_id: {}", artifacts.session_id).map_err(|_| StorageError::WriteFailed)?;
        writeln!(f, "total_bytes: {}", artifacts.total_bytes).map_err(|_| StorageError::WriteFailed)?;
        // store duration as seconds
        writeln!(f, "duration_secs: {}", artifacts.duration.num_seconds()).map_err(|_| StorageError::WriteFailed)?;
        Ok(())
    }
    ...
    fn get_capture_artifacts(&self, session_id: Uuid) -> Result<CaptureArtifacts, StorageError> {
        ...
        let read_bin = |name: &str| -> Result<Vec<u8>, StorageError> {
            let mut buf = Vec::new();
            File::open(dir.join(name)).and_then(|mut f| f.read_to_end(&mut buf)).map_err(|_| StorageError::ReadFailed)?;
            Ok(buf)
        };
        ...
        File::open(dir.join("tcp_timestamps.csv")).and_then(|mut f| f.read_to_string(&mut s)).map_err(|_| StorageError::ReadFailed)?;
        for line in s.lines() {
            let parts: Vec<_> = line.split(',').map(|x| x.trim()).collect();
            if parts.len() != 3 { continue; }
            let ts = DateTime::parse_from_rfc3339(parts[0]).map_err(|_| StorageError::ReadFailed)?.with_timezone(&Utc);
            let dirn = match parts[1] { "C2S" => Direction::ClientToContainer, "S2C" => Direction::ContainerToClient, _ => Direction::ClientToContainer };
            let sz = parts[2].parse::<usize>().map_err(|_| StorageError::ReadFailed)?;
            tcp_timestamps.push((ts, dirn, sz));
        }

        let mut stdio_timestamps: Vec<(DateTime<Utc>, StdioStream, usize)> = Vec::new();
        s.clear();
        File::open(dir.join("stdio_timestamps.csv")).and_then(|mut f| f.read_to_string(&mut s)).map_err(|_| StorageError::ReadFailed)?;
        for line in s.lines() {
            let parts: Vec<_> = line.split(',').map(|x| x.trim()).collect();
            if parts.len() != 3 { continue; }
            let ts = DateTime::parse_from_rfc3339(parts[0]).map_err(|_| StorageError::ReadFailed)?.with_timezone(&Utc);
        ...
        // meta
        s.clear();
        File::open(dir.join("meta.txt")).and_then(|mut f| f.read_to_string(&mut s)).map_err(|_| StorageError::ReadFailed)?;
