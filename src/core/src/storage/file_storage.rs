...
/// - `sessions/` — one `<uuid>.session` file per session (KV text)
/// - `interactions/` — one `<uuid>.bin` concatenating interaction bytes
/// - `artifacts/<uuid>/` — per-session directory with `*.bin`, `*.csv`, and `meta.txt`
...
impl FileStorage {
    /// Create a `FileStorage` rooted at `base_path`.
    ///
    /// The necessary subdirectories are created if missing.
    ...
    fn interactions_dir(&self) -> PathBuf {
        self.base_path.join("interactions")
    }
    fn artifacts_dir_for(&self, id: Uuid) -> PathBuf {
        self.artifacts_path.join(id.to_string())
    }

    fn session_file_path(&self, id: Uuid) -> PathBuf {
        self.sessions_dir().join(format!("{}.session", id))
    }
    ...
    fn write_session_file(&self, session: &Session) -> Result<(), StorageError> {
        ...
        writeln!(f, "id: {}", session.id).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
        writeln!(f, "service_name: {}", session.service_name).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
        writeln!(f, "client_addr: {}", session.client_addr).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
        writeln!(f, "start_time: {}", session.start_time.to_rfc3339()).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
        if let Some(end) = session.end_time {
            writeln!(f, "end_time: {}", end.to_rfc3339()).map_err(|e| {
                error!("Failed to write session file {}: {}", path.display(), e);
                StorageError::WriteFailed
            })?;
        } else {
            writeln!(f, "end_time: none").map_err(|e| {
                error!("Failed to write session file {}: {}", path.display(), e);
                StorageError::WriteFailed
            })?;
        }
        if let Some(ref cid) = session.container_id {
            writeln!(f, "container_id: {}", cid).map_err(|e| {
                error!("Failed to write session file {}: {}", path.display(), e);
                StorageError::WriteFailed
            })?;
        } else {
            writeln!(f, "container_id: none").map_err(|e| {
                error!("Failed to write session file {}: {}", path.display(), e);
                StorageError::WriteFailed
            })?;
        }
        writeln!(f, "bytes_transferred: {}", session.bytes_transferred).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
        ...
        writeln!(f, "status: {}", status_str).map_err(|e| {
            error!("Failed to write session file {}: {}", path.display(), e);
            StorageError::WriteFailed
        })?;
    ...
    fn parse_session_file(&self, path: &Path) -> Result<Session, StorageError> {
        ...
        let id = map
            .get("id")
            .ok_or_else(|| {
                error!("Missing id in session file {}", path.display());
                StorageError::ReadFailed
            })?
            .parse::<Uuid>()
            .map_err(|e| {
                error!("Invalid UUID in {}: {}", path.display(), e);
                StorageError::ReadFailed
            })?;
        let service_name = map.remove("service_name").ok_or_else(|| {
            error!("Missing service_name in {}", path.display());
            StorageError::ReadFailed
        })?;
        let client_addr_str = map.remove("client_addr").ok_or_else(|| {
            error!("Missing client_addr in {}", path.display());
            StorageError::ReadFailed
        })?;
        let client_addr = client_addr_str.parse().map_err(|e| {
            error!("Invalid client_addr in {}: {}", path.display(), e);
            StorageError::ReadFailed
        })?;
        let start_time = map
            .remove("start_time")
            .ok_or_else(|| {
                error!("Missing start_time in {}", path.display());
                StorageError::ReadFailed
            })
            .and_then(|s| {
                DateTime::parse_from_rfc3339(&s)
                    ...
                    .map_err(|e| {
                        error!("Invalid start_time in {}: {}", path.display(), e);
                        StorageError::ReadFailed
                    })
            })?;
        let end_time = map
            ...
            .map(|s| {
                DateTime::parse_from_rfc3339(&s)
                    .map(|dt| dt.with_timezone(&Utc))
                    .map_err(|e| {
                        error!("Invalid end_time in {}: {}", path.display(), e);
                        StorageError::ReadFailed
                    })
            })
            .transpose()?;
        ...
        let bytes_transferred = map
            ...
            .ok_or_else(|| {
                error!("Missing bytes_transferred in {}", path.display());
                StorageError::ReadFailed
            })?
            .parse::<u64>()
            .map_err(|e| {
                error!("Invalid bytes_transferred in {}: {}", path.display(), e);
                StorageError::ReadFailed
            })?;
        let status = match map
            .remove("status")
            .ok_or_else(|| {
                error!("Missing status in {}", path.display());
                StorageError::ReadFailed
            })?
            .as_str()
        {
...
impl Storage for FileStorage {
        ...
        if let Some(f) = filter {
            sessions.retain(|s| {
                ...
                if let Some(end) = f.end_date {
                    if s.end_time.unwrap_or(s.start_time) > end {
                        return false;
                    }
                }
                // client ip
                if let Some(ip) = f.client_addr {
                    if s.client_addr.ip() != ip {
                        return false;
                    }
                }
                // status
    ...
    fn save_capture_artifacts(&self, artifacts: &CaptureArtifacts) -> Result<(), StorageError> {
        ...
        f.write_all(&artifacts.tcp_client_to_container)
            .map_err(|e| {
                error!(
                    "Write failed: {}: {}",
                    sanitize_path(&dir.join("tcp_client_to_container.bin")),
        ...
        f.write_all(&artifacts.tcp_container_to_client)
            .map_err(|e| {
                error!(
                    "Write failed: {}: {}",
                    sanitize_path(&dir.join("tcp_container_to_client.bin")),
        ...
        for (ts, dirn, sz) in &artifacts.tcp_timestamps {
            let d = match dirn {
                Direction::ClientToContainer => "C2S",
                Direction::ContainerToClient => "S2C",
            };
    ...
    fn get_capture_artifacts(&self, session_id: Uuid) -> Result<CaptureArtifacts, StorageError> {
        ...
        File::open(dir.join("tcp_timestamps.csv"))
        ...
        for line in s.lines() {
            ...
            let ts = DateTime::parse_from_rfc3339(parts[0])
                .map_err(|e| {
                    error!("Invalid timestamp in tcp_timestamps.csv: {}", e);
                    StorageError::ReadFailed
                })?
                .with_timezone(&Utc);
            let dirn = match parts[1] {
                "C2S" => Direction::ClientToContainer,
                "S2C" => Direction::ContainerToClient,
                _ => Direction::ClientToContainer,
            };
            let sz = parts[2].parse::<usize>().map_err(|e| {
                error!("Invalid size in tcp_timestamps.csv: {}", e);
                StorageError::ReadFailed
            })?;
        ...
        s.clear();
        File::open(dir.join("stdio_timestamps.csv"))
            .and_then(|mut f| f.read_to_string(&mut s))
            .map_err(|e| {
                error!(
                    "Read failed {}: {}",
        ...
        for line in s.lines() {
            ...
            let ts = DateTime::parse_from_rfc3339(parts[0])
                .map_err(|e| {
                    error!("Invalid timestamp in stdio_timestamps.csv: {}", e);
                    StorageError::ReadFailed
                })?
                .with_timezone(&Utc);
            let stream = match parts[1] {
                "Stdin" => StdioStream::Stdin,
                "Stdout" => StdioStream::Stdout,
                _ => StdioStream::Stderr,
            };
            let sz = parts[2].parse::<usize>().map_err(|e| {
                error!("Invalid size in stdio_timestamps.csv: {}", e);
                StorageError::ReadFailed
            })?;
        ...
        // meta
        s.clear();
        File::open(dir.join("meta.txt"))
            .and_then(|mut f| f.read_to_string(&mut s))
            .map_err(|e| {
                error!("Read failed {}: {}", dir.join("meta.txt").display(), e);
                StorageError::ReadFailed
            })?;
...
mod tests {
    ...
    fn test_capture_artifacts_roundtrip() {
        ...
        assert_eq!(
            got.tcp_client_to_container,
            artifacts.tcp_client_to_container
        );
        assert_eq!(
            got.tcp_container_to_client,
            artifacts.tcp_container_to_client
        );
